...
1. Loads a raw CO₂ data file from the Mauna Loa Observatory's flas
sampling dataset, handling missing values and metadata.
2. Extracts the relevant columns: Year, Month, and CO₂ concentration
(ppm).
3. Saves the processed data to multiple formats:
   - CSV file for general use.
   - JSON file in ISO format, with an additional JSON file excluding
   NaN values.
...
File Structure:
    ...
    - Plot:
        - A time series plot of CO₂ concentration over time, created 
        with matplotlib.

Usage:
    - Call this module to load and process the data 
    from 'flask_monthly_raw.txt'.
    - Processed data files will be saved in the specified output 
    formats.
    - Run the plotting section to visualize CO₂ concentration as a 
    function of time.
"""
...
df = pd.read_csv('mauna-loa-data/flask_monthly_raw.txt', delim_whitespace=True,
...
df_extracted.to_json('mauna-loa-data/flask_monthly.json',
                     orient='records', date_format='iso')

# Create a filtered JSON file without NaN values
df_extracted.dropna().to_json('mauna-loa-data/flask_monthly_no_nan.json',
                              orient='records', date_format='iso')
